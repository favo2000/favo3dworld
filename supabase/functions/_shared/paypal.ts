// Fixed per deployment; never accept the environment from browser input.
// Secrets/tokens/provider response bodies must never be logged.
export function paypalHandler(environment:'sandbox'|'live'){
...
async function oauth(){
 ...
 const id=Deno.env.get(sandbox?'PAYPAL_CLIENT_ID':'PAYPAL_LIVE_CLIENT_ID'),secret=Deno.env.get(sandbox?'PAYPAL_CLIENT_SECRET':'PAYPAL_LIVE_CLIENT_SECRET');
 if(!id||!secret)throw new Error('PAYPAL_CONFIG');
...
return async(req:Request)=>{
 ...
 try{
  ...
  if(body.action==='health'){await oauth();return reply(200,{sandbox,payment_environment:environment,connected:true});}
  ...
  const capability=await mac(service,'paypal-'+environment+'-v1:'+requestKey);
  ...
  if(order.payment_status==='paid'){
   if(order.payment_environment!==environment)throw new Error('PAYMENT_MISMATCH');
  ...
  if(remote.status==='COMPLETED'){
   ...
   await db('rpc/confirm_paypal','POST',{p_id:order.id,p_paypal_id:order.paypal_order_id,p_capture_id:captures[0].id,p_currency:'CHF',p_amount:order.total,p_environment:environment});order.payment_status='paid';
   return reply(200,receipt());
  }
  const href=remote.links?.find((l:any)=>['approve','payer-action'].includes(l.rel))?.href;
  let approvalURL=null;if(href){const url=new URL(href);if(url.protocol==='https:'&&url.hostname===approvalHost&&!url.username&&!url.password)approvalURL=url.href;}

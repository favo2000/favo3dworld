...
async function admin(){
 ...
 const client={from(table){let one=false,update=null;const filters={};const query={select(){return query;},order(){return query;},range(){return query;},in(){return query;},eq(k,v){filters[k]=v;return query;},single(){one=true;return query;},update(v){update=v;updates.push(v);return query;},then(resolve){if(update)return Promise.resolve({data:[{id:order.id,status:update.status}],error:null}).then(resolve);if(table==='CustomerPhotos'){assert.equal(filters.order_id,order.id);assert.equal(filters.status,'attached');}return Promise.resolve({data:table==='Orders'?(one?order:[order]):table==='OrderItems'?items:table==='Products'?[{id:15,image_url:'https://cdn.example.test/fire.png'}]:{object_path:'private-path.png'},error:null}).then(resolve);}};return query;},storage:{from(bucket){assert.equal(bucket,'customer-photos');return {download:async file=>{downloads++;assert.equal(file,'private-path.png');return {data:new w.Blob(['private']),error:null};}};}}};
 ...
 assert.match(host.textContent,/FIRE × 2/);assert.match(host.textContent,/Nicht bezahlt/);assert.match(host.textContent,/CHF 25.00/);assert.equal(host.querySelector('script'),null);assert.equal(host.querySelector('.admin-order-item-image').src,'https://cdn.example.test/fire.png');
 [...host.querySelectorAll('button')].find(b=>b.textContent==='Privates Kundenfoto anzeigen').click();await tick();assert.equal(downloads,1);assert.equal(host.querySelector('img:not(.admin-order-item-image)').src,'blob:private-admin');
 const select=[...host.querySelectorAll('select')].at(-1);select.value='Versendet';[...host.querySelectorAll('button')].find(b=>b.textContent==='Status speichern').click();await tick();assert.equal(JSON.stringify(updates),JSON.stringify([{status:'Versendet'}]));
